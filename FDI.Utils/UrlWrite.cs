namespace FDI.Utils
{
    ...
    {
        // News
        public static string News(string slug, int id)
        {
            return string.Format("/{0}-p{1}n{2}.html", slug, WebConfig.NewsDetail, id);
        }
        public static string Tags(string lang, string name, int id)
        {
            return string.Format("/{0}tag/{1}-p{2}c{3}", lang, name, WebConfig.News, id);
        }

        public static string Search()
        {
            return string.Format("search-p{0}c0", WebConfig.News);
        }
        ...
        public static string VideoDetail(string slug, int newsId)
        {
            return string.Format("/{0}-p{1}v{2}.html", slug, WebConfig.VideoDetail, newsId);
        }
    }
}
